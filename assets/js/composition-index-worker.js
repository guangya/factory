...
(function (window) {
	...
	function splitToRows(images, params = null) {
		...
		for(const image of images) {
			// 检测是否即将发生溢出
			if ( (row.x + parseFloat(image.rowSpan) + parseFloat(image.width)) > parseFloat(options.pageWith) - parseFloat(options.pageRightMargin) ) {
				row = appendNewRow(rows, { x: options.pageLeftMargin });
			}
	
			image.x = row.x + parseFloat(image.colSpan);
			image.rowIndex = rowIndex;
			row.images.push(image);
	
			// 更新行的插入点坐标
			row.x = image.x + parseFloat(image.width);
			if (row.height < parseFloat(image.height) + parseFloat(image.rowSpan)) row.height = parseFloat(image.height) + parseFloat(image.rowSpan);
		}
...
const OldComposer = {
	...
	splitToRows: function (images, params = null) {
		...
		for(const image of images) {
			// 检测是否即将发生溢出
			if ( (row.x + parseFloat(image.row_span) + parseFloat(image.width)) > options.pageWith - options.pageRightMargin ) {
				row = this.appendNewRow(rows, { x: options.pageLeftMargin });
			}
	
			image.x = row.x + parseFloat(image.col_span);
			row.images.push(image);
	
			// 更新行的插入点坐标
			row.x = image.x + parseFloat(image.width);
			if (row.height < parseFloat(image.height) + parseFloat(image.row_span)) row.height = parseFloat(image.height) + parseFloat(image.row_span);
		}
